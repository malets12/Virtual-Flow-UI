...
self.addEventListener("message", async (msg:any): Promise<void> => {
	...
	switch (message.action) {
		case Constant.WorkerAction.LOAD: {
			worker.load(message as Message.LoadRequest).then(result => self.postMessage(result));
			break;
		}
		case Constant.WorkerAction.CALCULATE: {
			worker.calculate(message as Message.CalculationRequest).then(result => self.postMessage(result));
			break;
		}
	}
}, false);