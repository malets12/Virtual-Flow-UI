class Key {
  ...
  constructor() {
    ...
    for (const entry: [string, [string, string]] of Object.entries(key)) {
      const dimensionName: string = entry[0];
      const dimensionValues: Map<string, string> = new Map();
      for (const innerEntry: [string, string] of Object.keys(entry)) {
        const letter: string = innerEntry[1];
        const value: string = innerEntry[2];
        if (value === "0") {
          dimensionsWithZero.add(dimensionName);
        }
        dimensionValues.set(letter, value);
      }
      result.set(dimensionName, dimensionValues);
    }
    this.map = result;
